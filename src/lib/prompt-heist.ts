...
class PromptVault {
  ...
  async loadPrompt(name: string): Promise<string> {
    ...
    if (this.cache.has(name)) {
      console.log(`[PromptVault] 💾 Cache hit: ${name}`);
    ...
    if (localPrompt) {
      ...
      console.log(`[PromptVault] 📁 Loaded from local: ${name}`);
      return localPrompt.content;
    }
    
    // Fallback: fetch from GitHub directly (development mode)
    console.log(`[PromptVault] 🌐 Fetching from GitHub: ${name}`);
  ...
  listAvailable(): string[] {
    ...
    if (!fs.existsSync(this.promptsDir)) {
      console.warn('[PromptVault] prompts/fabric/ directory not found. Run "npm run heist" first.');
      return patterns;
    }
  ...
  clearCache(): void {
    this.cache.clear();
    console.log('[PromptVault] Cache cleared');
  }
 ...
 */
export async function combinePatterns(
  patternNames: string[],
 ...
 */
export async function createHybridPrompt(
  fabricPattern: string,
  councilPersona: string,
