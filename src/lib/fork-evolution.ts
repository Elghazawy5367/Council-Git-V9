    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          console.warn('⚠️  Rate limit hit. Use GitHub token for higher limits.');
        }
        break;
      }
...
async function analyzeFork(
  ...
  try {
    ...
    const hasTests = newFiles.some(f => 
      f.includes('test') || 
      f.includes('spec') ||
      f.startsWith('tests/') ||
...
export async function analyzeForkEvolution(config: ForkAnalysisConfig): Promise<ForkEvolutionReport> {
  ...
  console.log('🍴 Fork Evolution - Starting analysis...');
  console.log(`   Repository: ${owner}/${repo}`);
  console.log(`   Min Stars: ${minStars}`);
  console.log(`   Max Forks: ${maxForks}`);
  
  // Fetch forks
  console.log('\n📡 Fetching forks...');
  const allForks = await fetchForks(owner, repo, githubToken);
  console.log(`   Found: ${allForks.length} total forks`);
  
  // Filter by stars
  const popularForks = allForks
    .filter(f => f.stargazers_count >= minStars)
    .slice(0, maxForks);
  
  console.log(`   Popular: ${popularForks.length} forks with ${minStars}+ stars`);
  
  // Analyze each fork
  console.log('\n🔍 Analyzing fork changes...');
  const analyzedForks: ForkFeature[] = [];
  
  for (let i = 0; i < popularForks.length; i++) {
    const fork = popularForks[i];
    console.log(`   [${i + 1}/${popularForks.length}] ${fork.owner.login}/${fork.name}...`);
  ...
  console.log(`\n✅ Analysis complete: ${analyzedForks.length} forks with new features`);
  ...
  const topFeatures = Array.from(featureCounts.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([feature, count]) => `${feature} (${count} forks)`);
...
Usage:
  npm run forks <owner/repo>              # Analyze repository forks
  npm run forks <owner/repo> --stars 10   # Min stars filter
  npm run forks <owner/repo> --max 100    # Max forks to analyze

Options:
  --stars <number>   Min stars for fork inclusion (default: 5)
  --max <number>     Max forks to analyze (default: 50)
  --token <token>    GitHub API token (recommended)
  --help, -h         Show this help

Examples:
  npm run forks facebook/react --stars 50
  npm run forks vercel/next.js --max 100
  npm run forks remix-run/remix --token YOUR_TOKEN

Tip: Use GitHub token to increase rate limits (60 → 5000/hour)
Set: export GITHUB_TOKEN=your_token_here
  ...
  if (!repoArg || !repoArg.includes('/')) {
    ...
    console.log('Example: npm run forks facebook/react');
    return;
  }
